...
//			this.url = new URL(null,url,new com.sun.net.ssl.internal.www.protocol.https.Handler());
            this.connectionTimeout = connectionTimeout;
            this.readTimeOut = readTimeOut;
        } catch (MalformedURLException e) {
            logger.error(String.format("HttpClient construction has exception:%s",e.getMessage()));
        }
    }
    ...
    private String response(final HttpURLConnection connection, String encoding)
        ...
        try {
            ...
            logger.info(String.format("HTTP Return Status-Code:[%s]",connection.getResponseCode()));
            return sb.toString();
        } catch (Exception e) {
            throw e;
        } finally {
    ...
    private HttpURLConnection createConnection(String encoding) throws ProtocolException {
        ...
        try {
            ...
            logger.error(String.format("HttpURLConnection Program has Exception:[%s]--%s",e.getMessage(),e));
            return null;
        }
    ...
    private HttpURLConnection createConnectionGet(String encoding) throws ProtocolException {
        ...
        try {
            ...
            logger.error(String.format("createConnectionGet method has Exception:[%s]--%s",e.getMessage(),e));
            return null;
        }
        httpURLConnection.setConnectTimeout(this.connectionTimeout);// 连接超时时间
        httpURLConnection.setReadTimeout(this.readTimeOut);// 读取结果超时时间
        ...
        if (null != requestParam && 0 != requestParam.size()) {
            for (Map.Entry<String, String> en : requestParam.entrySet()) {
                try {
                    ...
                    logger.error(String.format("getRequestParamString method has Exception:[%s]--%s",e.getMessage(),e));
                    return "";
                }
            }
            reqstr = sf.substring(0, sf.length() - 1);
        }
        logger.info(String.format("请求报文(已做过URLEncode编码):[%s]",reqstr));
        return reqstr;
    }
